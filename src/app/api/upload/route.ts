    ...
    for (const signature of signatures) {
      ...
      if (signature.every((byte, i) => buffer[i] === byte)) {
        if (type === "image/webp") {
          ...
          if (![0x57, 0x45, 0x42, 0x50].every((b, i) => buffer[8 + i] === b)) continue;
        }
        return { type, ext };
      }
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validatedImage) return NextResponse.json({ error: "JPEG、PNG、GIF、WebP形式の画像のみアップロードできます" }, { status: 400 });
    const extMap: Record<string, string> = { "image/jpeg": "jpg", "image/png": "png", "image/gif": "gif", "image/webp": "webp" };
    const fileName = `${session.user.id}-${crypto.randomBytes(8).toString("hex")}.${extMap[validatedImage.type] || "jpg"}`;
